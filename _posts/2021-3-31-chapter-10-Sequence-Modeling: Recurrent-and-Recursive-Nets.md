...
CNN은 어떤 격자형태의 이미지를 처리하는데 특화된 모델이라면, RNN은 순차적인 데이터(sequential data)를 처리하는데 특화되어있다. RNN의 특징으로 시간축의 서로 다른 여러 부분에서 매개변수들을 공유한다는 점이 있다. 매개변수 공유를 통해 다층 신경망을 형태가 서로 다른(길이가 서로 다른) example의 모형화에 적용할 수 있다. 
...
![_config.yml](/Users/fredmac/dev/deeeperlearning.github.io/_posts/{{ site.baseurl }}/assets/ch9/Fig10_2.png)
